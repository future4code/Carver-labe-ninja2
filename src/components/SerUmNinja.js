...
class SerUmNinja extends React.Component {
    ...
    cadastrarServico = () => {
        ...
        else if (this.state.inputPreco <= 0) { alert(`Insira um preço acima de R$ 0.`) }
        else if (this.state.inputPagamento.length === 0) { alert(`Escolhe ao menos uma forma de pagamento.`) }
        else if (!this.compararData()) { alert(`A data limite para a realizeção do serviço deve ser maior do que a data atual.`) }
        else {
            const novoServico = {
